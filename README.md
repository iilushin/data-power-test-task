...
- [Более детальное описание проекта](#более-детальное-описание-проекта)
...
2. Приведение структуры и формата данных, дедупликация данных
3. Загрузка подготовленных данных в таблицу SQLite
4. Выгрузка загруженных данных в csv-файл (опционально)

Приложение поддерживает:
* автоматическую инициализацию БД и таблицы при первом запуске;
* конфигурацию путей, наименований файлов, параметров запуска, настроек таблицы с помощью файла `configs/config.py`;
...
* работает только с одним входным файлом единовременно;
* нельзя создать составной Primary Key для таблицы в SQLite;
* при дедупликации по списку колонок в выборку попадает первая попавшаяся запись, нельзя указывать условие фильтрации;
* работает только с одной схемой таблицы, нельзя изменять схему уже существующей таблицы.
...
```
git clone https://github.com/iilushin/data-power-test-task.git
```

## Установка зависимостей

Создайте виртуальное окружение при необходимости:
...
* `db/db.py` - отвечает за взаимодействие с бд - создание таблиц и выполнение запросов;
...
Вся программа выполняет один простой процесс: чтение данных из json-файла -> обработка данных (дедупликация, форматирование) -> инициализация бд и таблицы (если их нет) -> загрузка данных в таблицу бд.
...
Найденные во входном файле дубликаты (с учетом имеющихся записей в таблице) не будут записаны.  
Как происходит дедупликация: получение строк с колонками `DEDUPLICATE_DATA_BY_COLUMNS` из таблицы -> фильтрация входных данных по комбинации указанных колонок -> запись уникальных строк в таблицу.  
